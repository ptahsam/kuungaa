...
class _GroupContainerState extends State<GroupContainer> {
  ...
  void initState() {
    ...
    itemRefPosts = database.reference().child('KUUNGAA').child('Posts')
        .orderByChild('post_category').equalTo('groupsfeed');
        ...
        itemBuilder:(_, DataSnapshot snapshot, Animation<double> animation, int index){
          if(snapshot.exists){
            if(index + 1 <= listPosts.length) {
              return Column(
                children: [
                  GroupPostMainContainer(post: listPosts[index],),
                  listPosts.length > 1 ?
            ...
            if(snapshot.connectionState == ConnectionState.done)
            {
              return ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index){
                  return Column(
                    children: [
                      GroupPostMainContainer(post: snapshot.data![index],),
  ...
  Future<List> getAllGroupPosts()  async {
...
class GroupPostMainContainer extends StatefulWidget {
  final Posts? post;


  const GroupPostMainContainer({
    Key? key,
    this.post
  }) : super(key: key);

  @override
  State<GroupPostMainContainer> createState() => _GroupPostMainContainerState();
}

class _GroupPostMainContainerState extends State<GroupPostMainContainer> {
