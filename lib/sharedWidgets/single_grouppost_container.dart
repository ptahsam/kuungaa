...
import 'widgets.dart';
class SingleGroupPost extends StatefulWidget {
  ...
  @override
  _SingleGroupPostState createState() => _SingleGroupPostState();
}

class _SingleGroupPostState extends State<SingleGroupPost> {

  List<Posts> listPosts = [];
  Query? itemRefPosts;
  bool _anchorToBottom = false;
  bool groupHasPosts = false;
  ...
  Widget build(BuildContext context) {
    return groupHasPosts?Container(
      ...
      /*FutureBuilder(
          future: getAllPostsInGroup(widget.groupid),
              ...
              if(snapshot.data!.isNotEmpty){
                return ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index){
                    return Column(
                      children: [
                        SingleGroupPostMainContainer(post: snapshot.data![index],),
                ...
                return const Center(
                  child: Text("No posts yet", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18,),),
                );
              }
            }
            else
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6.0,),
                Text("No posts in this group", textAlign: TextAlign.center,),
              ],
            ),
          ),
        ),
  ...
  Future<List> getAllPostsInGroup(String groupid)  async {

    List<Posts> allPosts = [];

    final Query query = FirebaseDatabase.instance.reference().child('KUUNGAA').child('Posts')
        .orderByChild('post_id').equalTo(groupid);
    //allPosts.clear();
...
class SingleGroupPostMainContainer extends StatefulWidget {
  final Posts? post;


  const SingleGroupPostMainContainer({
    Key? key,
    this.post
  }) : super(key: key);

  @override
  State<SingleGroupPostMainContainer> createState() => _SingleGroupPostMainContainerState();
}

class _SingleGroupPostMainContainerState extends State<SingleGroupPostMainContainer> {
